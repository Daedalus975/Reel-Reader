...
export const AdultBooks: React.FC = () => {
  ...
  useEffect(() => {
    setCurrentPage('/adult/books')
  }, [setCurrentPage])

  const adultBooks = useMemo(() => media.filter((m) => m.type === 'doujinshi' && m.isAdult), [media])

  const genres = useMemo(() => {
    const set = new Set<string>()
    adultBooks.forEach((m) => m.genres.forEach((g) => set.add(g)))
    return Array.from(set).sort()
  }, [adultBooks])

  const displayMedia = useMemo(() => {
    let filtered = [...adultBooks]
    ...
    return filtered
  }, [adultBooks, selectedGenre, favoritesOnly])
    ...
    <main className="pt-24 pb-20 px-6 md:px-10 lg:px-16">
      <div className="bg-red-900/20 border border-red-800 rounded-lg p-4 mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-red-400 font-semibold">Adult Books (18+)</p>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-4xl font-bold text-light mb-2">Adult Books</h1>
          <p className="text-gray-400 text-sm">{adultBooks.length} books</p>
        </div>
      ...
      {displayMedia.length === 0 && (
        <div className="text-center py-20">
          <p className="text-gray-400 text-lg">No adult books found</p>
        </div>
      )}

      <AddMediaModal isOpen={showAddModal} onClose={() => setShowAddModal(false)} mediaType={'doujinshi'} />
    </main>
  )
}
